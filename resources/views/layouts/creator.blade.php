        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="topnavLinks">
                <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
                    <li class="nav-item">
                        <a class="nav-link text-white {{ request()->routeIs('guest.home') ? 'active' : '' }}"
                           href="#" style="font-size:20px;">
                            Home
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-white {{ request()->routeIs('guest.about') ? 'active' : '' }}"
                           href="#" style="font-size:20px;">
                            Plans
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-white {{ request()->routeIs('guest.plans.index') ? 'active' : '' }}"
                           href="#" style="font-size:20px;">
                            Plans too
                        </a>
                    </li>
                ...
                <div class="ms-lg-auto d-flex align-items-center gap-3">
                    ...
                    @if($user)
                        <span class="badge text-bg-light">
                            ...
                            @if($user->hasRole('super-admin', null))
                                <span class="ms-1 badge bg-dark">Super Admin</span>
                            @elseif($tenantId && $user->hasRole('admin', $tenantId))
                                <span class="ms-1 badge bg-warning text-dark">Tenant Admin</span>
                            @elseif($tenantId && $user->hasRole('user', $tenantId))
                                <span class="ms-1 badge bg-secondary">User</span>
                            @endif
                        </span>
                    ...
                    @else
                        <a class="btn btn-sm btn-light" href="#">Log in</a>
                    @endif
                </div>
            </div>
        </div>
                    ...
                    <div class="card shadow-sm">
                        <div class="card-body">
                            <nav class="nav flex-column">
                                ...
                                {{-- ADMIN-ONLY (landlord super-admin) --}}
                                @if($user && $user->hasRole('super-admin', null))
                                    <a class="nav-link {{ request()->routeIs('landlord.home') ? 'active' : '' }}"
                                       href="#">
                                        <span class="me-2">🧭</span> Landlord Home
                                    </a>
                                @endif

                                {{-- TENANT-ADMIN extras --}}
                                @if($user && $tenantId && $user->hasRole('admin', $tenantId))
                                    <a class="nav-link {{ request()->routeIs('tenant.settings') ? 'active' : '' }}"
                                       href="#">
                                        <span class="me-2">⚙️</span> Tenant Settings
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('tenant.users.index') ? 'active' : '' }}"
                                       href="#">
                                ...
                                {{-- USER-only links --}}
                                @if($user && $tenantId && $user->hasRole('user', $tenantId))
                                    <a class="nav-link {{ request()->routeIs('tenant.profile') ? 'active' : '' }}"
                                       href="#">
            ...
            <div class="col-12 col-lg-9 col-xl-10">
                ...
                <div class="offcanvas offcanvas-start d-lg-none" tabindex="-1" id="tenantSidebar"
                    ...
                    <div class="offcanvas-body">
                        <nav class="nav flex-column">
                            ...
                            {{-- mirror key role links for mobile --}}
                            @if($user && $user->hasRole('super-admin', null))
                                <a class="nav-link {{ request()->routeIs('landlord.home') ? 'active' : '' }}"
                                   href="#">
                                    <span class="me-2">🧭</span> Landlord Home
                ...
                @hasSection('title')
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        ...
                        {{-- Example: admin-only page actions --}}
                        @if($user && ($user->hasRole('super-admin', null) || ($tenantId && $user->hasRole('admin', $tenantId))))
                            @yield('actions')
                        @endif
                    </div>
                @endif
